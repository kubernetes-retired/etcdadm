...
func WriteEtcdctlEnvFile(cfg *apis.EtcdAdmConfig) error {
	...
	environmentFileDir := filepath.Base(cfg.EtcdctlEnvFile)
	...
	f, err := os.OpenFile(cfg.EtcdctlEnvFile, os.O_RDWR|os.O_CREATE, 0644)
	if err != nil {
		return fmt.Errorf("unable to open the etcd environment file %s: %s", constants.EtcdctlEnvFile, err)
	}
	defer f.Close()
