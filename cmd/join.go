...
func init() {
	...
	joinCmd.PersistentFlags().StringVar(&etcdAdmConfig.UnitFile, "unit-file", "/etc/systemd/system/etcd.service", "system unit file")
	joinCmd.PersistentFlags().StringVar(&etcdAdmConfig.EnvironmentFile, "env-file", "/etc/etcd/etcd.env", "system env file")
	joinCmd.PersistentFlags().StringVar(&etcdAdmConfig.EtcdctlEnvFile, "ctl-file", "/etc/etcd/etcdctl.env", "system etcd ctl file")
