...
func init() {
	...
	initCmd.PersistentFlags().Int32Var(&etcdAdmConfig.ClientPort, "client-port", 2379, "etcd port listening from client")
	initCmd.PersistentFlags().Int32Var(&etcdAdmConfig.PeerPort, "peer-port", 2380, "etcd port listening from peer etcd instance")
	initCmd.PersistentFlags().StringVar(&etcdAdmConfig.BindAddr, "bind-address", "", "etcd bind address")
